...
image = cv2.imread("start_image.jpg")
focal_length = 0
# Объект для замера
actual_width = 9  # Фактическая ширина объекта в сантиметрах
# расстояние до объекта
distance = 30  # Расстояние до объекта в сантиметрах

# пороговые значения для выделение контура объекта (красный)
lower_threshold = np.array([42, 0, 0])  # Нижние значения диапазона цвета в HSV
upper_threshold = np.array([76, 255, 255])  # Верхние значения диапазона цвета в HSV
...
while True:
    ...
    # Применение пороговых значений для выделения объектов заданного (красный) цвета
